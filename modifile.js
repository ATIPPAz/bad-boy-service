...
async function createUniquePair({ header, body }, estimate) {
...
async function createPairTemplate(memberLength, estimate = 4) {
  const ids = createIdTeam(memberLength)
  const template = createTeamHeaderAndBody(ids)
  const result = await createUniquePair(template, estimate)
  return result
}
...
async function createTeamPair(member,limit,start,random,memberLock=[]) {
  return await new Promise(async(resolve, reject) => {
    try {
      const cateria = { memberLength: member.length, limit: limit, start: start, random: random }
      const result = (await createPairTemplate(cateria.memberLength, cateria.limit ?? null)).filter((_, index) => index + 1 > cateria.start)
      const ids = []
      result.forEach(x => {
        x.forEach(y => {
          y.forEach(v => {
